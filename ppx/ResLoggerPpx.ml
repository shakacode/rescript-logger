  ...
  let format ns ctx tag =
    ...
    | None ->
      (
        ...
        | `WithPayload -> Exp.constant (Pconst_string (tag ^ " with payload", None))
        | `WithNotLoggedPayload -> Exp.constant (Pconst_string (tag ^ " with payload (not logged)", None))
      )
    | Some ns ->
      let __ = "::" in
      match ctx with
      | `WithoutPayload -> Exp.constant (Pconst_string (ns ^ __ ^ tag, None))
      | `WithPayload -> Exp.constant (Pconst_string (ns ^ __ ^ tag ^ " with payload", None))
      | `WithNotLoggedPayload -> Exp.constant (Pconst_string (ns ^ __ ^ tag ^ " with payload (not logged)", None))
        ...
        fun str ->
          let ctx =
            Expansion_context.Base.top_level
              ...
              ~file_path: (str |> File_path.get_default_path_str) in
